...
class ResNet_FAKD(nn.Module):
    ...
    def __init__(self, depth, num_filters, teacher_features, teacher_sizes, block_name='BasicBlock', num_classes=10,
        ...
        if self.logit_based:
            self.flow4 = FlowAlignModule(num_classes, num_filters[3] * block.expansion, "logit_based", None, None,
    ...
    def forward(self, x, teacher_features=None, is_feat=False, preact=False, inference_sampling=4):
        ...
        x = self.layer1(x)  # 32x32
        loss, x = self.flow1(x, teacher_features[0], inference_sampling=inference_sampling)
        flow_loss += loss
        f1 = x
        x = self.layer2(x)  # 16x16
        loss, x = self.flow2(x, teacher_features[1], inference_sampling=inference_sampling)
        flow_loss += loss
        f2 = x
        x = self.layer3(x)  # 8x8
        loss, x = self.flow3(x, teacher_features[2], inference_sampling=inference_sampling)
        flow_loss += loss
        f3 = x
        ...
        f4 = x
        if self.logit_based:
            loss, x = self.flow4(x, teacher_features[3], inference_sampling=inference_sampling)
            flow_loss += loss
        else:
            x = self.fc(x)
        if is_feat:
...
def resnet20_fakd(**kwargs):
    return ResNet_FAKD(20, [16, 16, 32, 64], 'basicblock', **kwargs)
...
def resnet32_fakd(**kwargs):
    return ResNet_FAKD(32, [16, 16, 32, 64], 'basicblock', **kwargs)
...
def resnet8x4_fakd(**kwargs):
    return ResNet_FAKD(8, [32, 64, 128, 256], 'basicblock', **kwargs)
